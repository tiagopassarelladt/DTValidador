...
type
  TDocumentType = (dtCPF, dtCNPJ, dtCNPJAlfanumerico, dtCNPJAuto);
  ...
  private
    ...
    FIsValid: Boolean;
    FDetectedDocType: TDocumentType;
    ...
    function DetectarTipoCNPJ(const ACNPJ: string): TDocumentType;
  ...
  public
    ...
    property DetectedDocType: TDocumentType read FDetectedDocType;
...
function TDTValidador.DetectarTipoCNPJ(const ACNPJ: string): TDocumentType;
var
  CNPJLimpo: string;
  i: Integer;
  TemLetras: Boolean;
begin
  ...
  // Verifica se o comprimento é válido para um CNPJ
  if Length(CNPJLimpo) <> CNPJ_LENGTH then
  begin
    // Se o comprimento não for 14, não é um CNPJ válido
    // Vamos retornar dtCNPJ como padrão, e a validação irá falhar depois
    Result := dtCNPJ;
    Exit;
  end;

  // Verifica se há letras nas 12 primeiras posições
  TemLetras := False;
  for i := 1 to CNPJ_BASE_LENGTH do
  begin
    if CharInSet(CNPJLimpo[i], ['A'..'Z']) then
    begin
      TemLetras := True;
      Break;
    end;
  ...
  // Verifica se os 2 últimos dígitos são numéricos
  if not (CharInSet(CNPJLimpo[13], ['0'..'9']) and CharInSet(CNPJLimpo[14], ['0'..'9'])) then
  begin
    // Se os 2 últimos dígitos não forem numéricos, não é um CNPJ válido
    Result := dtCNPJ; // Padrão, validação irá falhar depois
    Exit;
  end;

  // Determina o tipo com base na presença de letras
  if TemLetras then
    Result := dtCNPJAlfanumerico
  else
    Result := dtCNPJ;
end;
...
var
  DocLimpo: string;
  TipoDetectado: TDocumentType;
begin
  DocLimpo := RemoveMascara(ADocument);
  DocLimpo := UpperCase(DocLimpo);

  // Se o tipo for dtCNPJAuto, detectar o tipo
  if AType = dtCNPJAuto then
  begin
    if Length(DocLimpo) = CPF_LENGTH then
      TipoDetectado := dtCPF
    else
      TipoDetectado := DetectarTipoCNPJ(DocLimpo);
  end
  else
    TipoDetectado := AType;

  case TipoDetectado of
    dtCPF:
      begin
        if Length(DocLimpo) = CPF_LENGTH then
    ...
    dtCNPJ, dtCNPJAlfanumerico, dtCNPJAuto:
      begin
...
begin
  ...
  // Se o tipo for dtCNPJAuto, determinar automaticamente o tipo de CNPJ
  if FDocumentType = dtCNPJAuto then
  begin
    FDetectedDocType := DetectarTipoCNPJ(FDocument);

    case FDetectedDocType of
      dtCNPJ:
      ...
      dtCNPJAlfanumerico:
        ...
        begin
          FLastError := 'Tipo de CNPJ não identificado.';
          FIsValid := False;
        end;
    end;
  end
  ...
  begin
    // Processamento normal para os outros tipos de documento
    FDetectedDocType := FDocumentType;
